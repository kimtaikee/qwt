...
public:
    ...
    bool isHorizontalDroplineVisible = false;
    bool isVerticalDroplineVisible = false;
    bool isLabelVisible = false;

    float droplineOpacity = 1.0;
    float labelOpacity = 1.0;
    ...
    QwtPlotCurve::LabelPosition labelPos = QwtPlotCurve::Center;
...
void QwtPlotCurve::drawDropLines(QPainter *painter, const QPolygonF &ploygon, const QRectF &canvas) const
{
    if (!d_data->isHorizontalDroplineVisible && !d_data->isVerticalDroplineVisible)
        return;

    painter->save();
    painter->setRenderHints(QPainter::Antialiasing, false);
    painter->setPen(d_data->droplinePen);
    painter->setOpacity(d_data->droplineOpacity);

    for (int i = 0; i < ploygon.size(); ++i) {
        const QPointF p = ploygon.at(i);

        // draw vertical line
        if (d_data->isVerticalDroplineVisible)
            painter->drawLine(p, QPoint(p.x(), canvas.height()));

        // draw horizontal line
        if (d_data->isHorizontalDroplineVisible)
        painter->drawLine(QPoint(0, p.y()), p);
    }
    painter->restore();
}

void QwtPlotCurve::drawLabels(QPainter *painter, const QwtSymbol &,
                              const QwtScaleMap &xMap, const QwtScaleMap &yMap,
                              const QRectF &canvasRect, int from, int to ) const
{
    if (!d_data->isLabelVisible)
        return;
    ...
    for ( int i = from; i <= to; i += chunkSize )
    {
        ...
        drawLabels_(painter, points, *data());
    }
}

void QwtPlotCurve::drawLabels_(QPainter *painter, const QPolygonF &points, const QwtSeriesData<QPointF>& data) const
{
    Q_ASSERT(points.size() == data.size());
    painter->save();
    painter->setPen(d_data->labelColor);
    painter->setFont(d_data->labelFont);
    painter->setOpacity(d_data->labelOpacity);

    for (int i = 0; i < points.size(); ++i) {
        const QString text = QString("%1").arg(data.sample(i).y());
        qreal textWidth = plot()->fontMetrics().width(text);
        qreal textHeight = plot()->fontMetrics().height();
        const QPointF point = points.at(i);
        switch (d_data->labelPos) {
        case Center:
            painter->drawText(QPointF(point.x() - textWidth / 2 + d_data->labelOffsetX,
                                      point.y() + textHeight / 2 + d_data->labelOffsetY),
                                      text);
            break;

        case Left:
            painter->drawText(QPointF(point.x() - textWidth + d_data->labelOffsetX,
                                      point.y() + textHeight / 2 + d_data->labelOffsetY),
                                      text);
            break;

        case Right:
            painter->drawText(QPointF(point.x() + d_data->labelOffsetX,
                                      point.y() + textHeight / 2 + d_data->labelOffsetY),
                                      text);
            break;
        ...
        case Below:
            painter->drawText(QPointF(point.x() - textWidth / 2 + d_data->labelOffsetX,
                                      point.y() + textHeight + d_data->labelOffsetY),
                                      text);
            break;

        case Above:
            painter->drawText(QPointF(point.x() - textWidth / 2 + d_data->labelOffsetX,
                                      point.y() - textHeight + d_data->labelOffsetY), text);
            break;
        }
    }
    painter->restore();
...
void QwtPlotCurve::setHorizontalDroplineVisible(bool visible)
{
    d_data->isHorizontalDroplineVisible = visible;
}

bool QwtPlotCurve::isHorizontalDroplineVisible() const
{
    return d_data->isHorizontalDroplineVisible;
}

void QwtPlotCurve::setVerticalDroplineVisible(bool visible)
{
    d_data->isVerticalDroplineVisible = visible;
}

bool QwtPlotCurve::isVerticalDroplineVisible() const
{
    return d_data->isVerticalDroplineVisible;
}

void QwtPlotCurve::setDroplinePen(const QPen& pen)
{
    d_data->droplinePen = pen;
}

QPen QwtPlotCurve::droplinePen() const
{
    return d_data->droplinePen;
}

void QwtPlotCurve::setDroplineColor(const QColor& clr)
{
    d_data->droplinePen.setColor(clr);
}

QColor QwtPlotCurve::droplineColor() const
{
    return d_data->droplinePen.color();
}

void QwtPlotCurve::setDroplineOpacity(float opacity)
{
    d_data->droplineOpacity = opacity;
}

float QwtPlotCurve::droplineOpacity() const
{
    return d_data->droplineOpacity;
}

void QwtPlotCurve::setLabelVisible(bool visible)
{
    d_data->isLabelVisible = visible;
}

bool QwtPlotCurve::isLabelVisible() const
{
    return d_data->isLabelVisible;
}

void QwtPlotCurve::setLabelFont(const QFont& f)
{
    d_data->labelFont = f;
}

QFont QwtPlotCurve::labelFont() const
{
    return d_data->labelFont;
}

void QwtPlotCurve::setLabelOffsetX(int offset)
{
    d_data->labelOffsetX = offset;
}

int QwtPlotCurve::labelOffsetX() const
{
    return d_data->labelOffsetX;
}

void QwtPlotCurve::setLabelOffsetY(int offset)
{
    d_data->labelOffsetY = offset;
}

int QwtPlotCurve::labelOffsetY() const
{
    return d_data->labelOffsetY;
}

void QwtPlotCurve::setLabelPosition(LabelPosition pos)
{
    d_data->labelPos = pos;
}

QwtPlotCurve::LabelPosition QwtPlotCurve::labelPosition() const
{
    return d_data->labelPos;
}

void QwtPlotCurve::setLabelOpacity(float opacity)
{
    d_data->labelOpacity = opacity;
}

float QwtPlotCurve::labelOpacity() const
{
    return d_data->labelOpacity;
}

void QwtPlotCurve::setLabelColor(const QColor& clr)
{
    d_data->labelColor = clr;
}

QColor QwtPlotCurve::labelColor() const
{
    return d_data->labelColor;
}
    ...
    const QRectF &canvasRect, int from, int to ) const
{
    ...
    if ( qwtVerifyRange( numSamples, from, to ) > 0 )
    {
        ...
        drawLabels( painter, *d_data->symbol,
                   xMap, yMap, canvasRect, from, to );
    }
}
    ...
    const QRectF &canvasRect, int from, int to ) const
{
    ...
    if ( doIntegers )
    {
        ...
        if ( d_data->paintAttributes & ClipPolygons )
        {
            ...
            drawDropLines( painter, clipped, canvasRect );

        }
        else
        {
            QwtPainter::drawPolyline( painter, polyline );
            drawDropLines(painter, polyline, canvasRect);
        }
    }
    else
    {
...
bool QwtPlotCurve::contains(const QPoint &pos)
{
    const int delta = 10;
    double distance;
    closestPoint(pos, &distance);
    qDebug() << "QwtPlotCurve: distance = " << distance;
