...
void QwtScaleDraw::drawLabel( QPainter *painter, double value ) const
{
    ...
    int dw = decimalWidth();
    if ( dw != 0) {
        if (!isMinusSignVisible() && value < 0) {
            lbl.setText(QString("%1%2%3").arg(prefix().trimmed()).
                        arg(-value, 4, 'f', dw, '0').arg(suffix().trimmed()));
        } else if (isPlusSignVisible() && value > 0){
            lbl.setText(QString("%1+%2%3").arg(prefix().trimmed()).
                        arg(lbl.text().toInt(), 4, 'f', dw, '0').arg(suffix().trimmed()));
        } else {
            QString("%1%2%3").arg(prefix().trimmed()).
                    arg(lbl.text().toInt(), 4, 'f', dw, '0').arg(suffix().trimmed());
        }
    } else {
        if (!isMinusSignVisible() && value < 0) {
            lbl.setText(QString("%1%2%3").arg(prefix().trimmed()).
                        arg(-value).arg(suffix().trimmed()));
        } else if (isPlusSignVisible() && value > 0){
            lbl.setText(QString("%1+%2%3").arg(prefix().trimmed()).
                        arg(lbl.text()).arg(suffix().trimmed()));
        } else {
            QString("%1%2%3").arg(prefix().trimmed()).
                    arg(lbl.text()).arg(suffix().trimmed());
        }
