...
public:
    ...
    QMap<int, QwtColumnRect> sampleColumnRectMap;
};
...
/*!
 * \return The bounding rect of the bar.
 */
QwtColumnRect QwtPlotBarChart::columnRect(int index) const
{
    return d_data->sampleColumnRectMap.value(index);
}

bool QwtPlotBarChart::contains(const QPoint &pos) const
{
    bool in = false;
    QMap<int, QwtColumnRect>::const_iterator iter = d_data->sampleColumnRectMap.begin();
    while (iter != d_data->sampleColumnRectMap.end()) {
        if (iter.value().toRect().contains(pos)) {
            in = true;
            break;
        }
        ++iter;
    ...
    int index, const QPointF &sample ) const
{
    ...
    if (d_data->sampleColumnRectMap.contains(index))
    {
        d_data->sampleColumnRectMap[index] = barRect;
    }
    else
    {
        d_data->sampleColumnRectMap.insert(index, barRect);
    }
...
{
    QwtColumnSymbol *specialSym =
        specialSymbol( sampleIndex, sample );

    QwtColumnSymbol *sym = specialSym;
    if ( sym == NULL )
        sym = d_data->symbol;

    // custom bar chart features
    QPalette pal = sym->palette();
    QBrush b = pal.window();
    b.setStyle(plot()->barChartFillStyle());
    pal.setBrush(QPalette::Window, b);
    sym->setPalette(pal);
    sym->setLineWidth(plot()->barChartLineWidth());
    painter->setOpacity(plot()->barChartOpacity());
    painter->setPen(plot()->barChartBorderPen());
