...
void QwtPlotMultiBarChart::setFloatingBar(bool floating)
{
    d_data->isFloatingBar = floating;
}

bool QwtPlotMultiBarChart::isFloatingBar() const
{
    return d_data->isFloatingBar;
}
...
bool QwtPlotMultiBarChart::contains(const QPoint &pos)
{
    for (int i = 0; i < d_data->barRects.size(); ++i)
        if (d_data->barRects.at(i).toRect().contains(pos))
            return true;
    ...
    const QwtSetSample& sample ) const
{
    ...
    if ( orientation() == Qt::Vertical )
    {
        ...
        for ( int i = 0; i < numBars; i++ )
        {
            ...
            d_data->barRects.append(barRect);
        }
    }
    else
    {
